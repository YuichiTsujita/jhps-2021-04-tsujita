...
is another optimization associated
with the I/O throttling, where congestion of data transfer
among compute nodes can be mitigated by stepwise data aggregations
issued from younger number group.
In Fig.~\ref{fig:IO_THROT_DATA_AGG}\ref{fig:STEP_WISE_AGG},
processes in the first group numbered as 0 issues data transfers
to aggregators on every group, where we describe them
such as (0,0), (0,1), and (0,7) in the first step.
In the first step, only the (0,0) is carried out because other groups
are not ready in data aggregation.
In the next step, the second group issues data aggregation
denoted such as (0,1), (1,1), and (7,1), and data aggregations
for (0,1) and (1,1) are done in this step.
In the last step, the last group enters in data aggregation phase
with every group.
