...
Holistic log analysis has been proposed in order to investigate
I/O performance bottlenecks or to tune I/O performance of
applications~\cite{lockwood:cug18,yang:nsdi2019}.
...
The PostgreSQL database server has collected information of executed jobs
at the K computer on the JOB-DB database.
The JOB-DB keeps used compute nodes, elapsed time, and start and finish times
of job execution, for instance by associating with a JOB-ID.
Therefore, we can refer to information about a target job from the JOB-DB
by specifying a JOB-ID.
...
Figure~\ref{fig:IO_THROT_DATA_AGG}\ref{fig:IO_THROT} illustrates
...
Fig.~\ref{fig:IO_THROT_DATA_AGG}\ref{fig:STEP_WISE_AGG}
...
of the EARTH.
In this paper, {\tt original} stands for the original MPI-IO, while a combination of
...
I/O operations as shown in Fig.~\ref{fig:IOR_HINTS}.
%
\begin{figure}[tb]
\begin{verbatim}
...
\end{figure}
%
The first line set the number of aggregators as eight times of stripe count,
thus 1,536 processes acted as aggregators.
The second line informed MPI-IO implementation that there were 4 processes
per compute node.
...
is used, and we can monitor Tofu data transfer packet status through TNRs of TofuD.
Unfortunately, we did not have any chance to relate real application jobs with
the proposed framework in the K computer because the implementation and the evaluation
were done only for the last few months before the K computer termination as trial.
...
We did not have any enhanced work about the proposed framework
on other HPC platforms unfortunately.
However, we consider that the framework can be easily enhanced
since the framework has been built by open source environment
such as {\itshape Python}.
Although it is partially lack of generality by using logs of Tofu and FEFS,
which are specific in Fujitsu's machine,
we can enhance its portability by having an abstract layer
on top of underlying system-dependent interface layer.
Other interconnects such as Gemini or Aries from Cray
also provide the similar hardware counters, and
they have been used in log analysis
studies~\cite{chunduri:pmbs19,ahlgren:cug18,zimmer:cug16,pedretti:cug13}.
Besides, metrics extracted from FEFS are ones derived from Lustre,
and they are also available in Lustre technology.
