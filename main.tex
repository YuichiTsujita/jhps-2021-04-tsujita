...
%
\begin{itemize}
\item Three metrics of {\tt I/O stats}: ({\tt req\_qdepth}, {\tt req\_waittime}, and {\tt req\_active})
\item Two metrics of {\tt Tofu stats}: ($R_{BW}$ and $T_{wait}^{max}$)
\item Mean OST bandwidth obtained from write and read OST throughput values
measured in {\tt I/O rates} 
\end{itemize}
%
We gave them ranks from 1 in the order from the best one
among the evaluated optimizations.
...
\hline
\multicolumn{1}{c}{Optimization}  & \multicolumn{3}{c}{I/O stats} & \multicolumn{2}{c}{Tofu stats} & Mean OST & Overall \\
\multicolumn{1}{c}{configuration} & {\tt req\_qdepth} & {\tt req\_waittime} & {\tt req\_active} &  $R_{BW}$ & $T_{wait}^{max}$ & bandwidth & score \\
\hline
